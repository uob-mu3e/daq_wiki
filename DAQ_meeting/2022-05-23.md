...
* Prompted by https://elog.psi.ch/elogs/Testbeams/4244, where do we put in calibration for sensor readout. common strategy would be nice. Options:
1) MSCB driver 2) ODB generic 3) History and custom pages (twice the same info!) 4) ?  .
I don`t like to only have the calibrated data available, as one need to reverse engineer to recalibrate.
